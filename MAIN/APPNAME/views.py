        ...
        if "signup-form" in request.POST:
            ...
            if password != password2:
                ...
                if new_user is not None:
                    ...
                    msg = f"{username}. Thanks for subscribing, welcome to password mangager."
            ...
            to_delete = request.POST.get("password-id")
            print(f"\n\nto_delete>>>>>>>>>>>>>>{to_delete}\n\n")
            value = request.POST.get("passwordvalue-id")
            print(f"\n\nvalue>>>>>>>>>>>>{value}\n\n")
            print(f"\n\npasswordvalue-hidden>>>>>>>>>>>{request.POST.get('passwordvalue-hidden')}\n\n")
            msg = f"{Password.objects.get(id=to_delete).name} deleted."
            Password.objects.get(id=to_delete).delete()
            messages.success(request, msg)
            return HttpResponseRedirect(request.path)
